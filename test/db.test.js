...
routes.forEach(route => {
  describe(`db with ${route}`, () => {

    const table = route === 'v1.1' ? "analytics_data" : "analytics_data_ga4"
    const queryVersion = route === `v1.1` ? '1.1' : '2'

    beforeEach(done => {
      databaseSupport.resetSchema(table).then(() => done());
    });

    describe('.query(params)', () => {
      it('should return all rows for the given agency and report', done => {
        databaseSupport.client(table).insert([
          ...
          return db.query({ reportName: 'my-report', reportAgency: 'my-agency', version: queryVersion });
        }).then(results => {
    ...
    //       { report_name: 'my-report', report_agency: null }
    //     ]).then(() => {
    //       return db.query({ reportName: 'my-report' , version: queryVersion });
    //     }).then(results => {
    //       expect(results).to.have.length(1);
    //       expect(results[0].report_name).to.equal('my-report');
    ...
    //     });
    //     databaseSupport.client(table).insert(rows).then(() => {
    //       return db.query({ reportName: 'report', limit: 4 , version: queryVersion});
    //     }).then(results => {
    ...
    //     });
    //     databaseSupport.client(table).insert(rows).then(() => {
    //       return db.query({ reportName: 'report', limit: 11000 , version: queryVersion});
    //     }).then(results => {
    ...
    //     });
    //     databaseSupport.client(table).insert(rows).then(() => {
    //       return db.query({ reportName: 'report', limit: 3, page: 1 , version: queryVersion });
    //     }).then(results => {
    //       expect(results).to.have.length(3);
    //       expect(results[0].date.toISOString()).to.match(/^2017-01-06/);
    //       expect(results[2].date.toISOString()).to.match(/^2017-01-04/);

    //       return db.query({ reportName: 'report', limit: 3, page: 2 , version: queryVersion});
    //     })
    //     .then(results => {
    //       expect(results).to.have.length(3);
    ...
    //       ).then(() => {
    //         return db.queryDomain('test.gov', 'site', 2, 1, null, null, table);
    //       }).then(results => {
    ...
    //         { report_name: 'site', date: '2017-01-03', data: { domain: 'test.gov' } }]
    //     ).then(() => {
    //       return db.queryDomain('test.gov', 'site', 1000, 1, null, null, table);
    //     }).then(results => {
    //       expect(results).to.have.length(2);
    ...
    //     ).then(() => {
    //       return db.queryDomain('test.gov', 'site', 1000, 1, '2017-10-20', null, table);
    //     }).then(results => {
    //       expect(results).to.have.length(2);
    //       expect(results[0].report_name).to.equal('site');
    //       expect(results[0].data.domain).to.equal('test.gov');
    ...
    //         { report_name: 'site', date: '2018-01-03', data: { domain: 'usda.gov' } }]
    //     ).then(() => {
    //       return db.queryDomain('test.gov', 'site', 1000, 1, null, '2017-10-20', table);
    //     }).then(results => {
    //       expect(results).to.have.length(1);
    //       expect(results[0].report_name).to.equal('site');
    //       expect(results[0].data.domain).to.equal('test.gov');
    ...
    //         { report_name: 'site', date: '2018-01-03', data: { domain: 'usda.gov' } }]
    //     ).then(() => {
    //       return db.queryDomain('test.gov', 'site', 1000, 1, '2018-01-02', '2017-10-20', table);
    //     }).then(results => {
    //       expect(results).to.have.length(2);
    //       expect(results[0].report_name).to.equal('site');
    //       expect(results[0].data.domain).to.equal('test.gov');
    ...
    //         { report_name: 'site', date: '2018-01-03', data: { domain: 'usda.gov' } }]
    //     ).then(() => {
    //       return db.queryDomain('test.gov', 'site', 1000, 1, '2018-01-04', '2017-10-20', table);
    //     }).then(results => {
    //       expect(results).to.have.length(2);
    //       expect(results[0].report_name).to.equal('site');
    //       expect(results[0].data.domain).to.equal('test.gov');
